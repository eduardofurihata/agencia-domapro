# DOMA PRO - Proposta de Valor e Estrutura do Produto

> Canal de assinantes focado em doma de cavalos - Fase inicial da esteira de produtos

---

## 1. O Que É o Doma Pro

### Definição
**Doma Pro** é o clube de assinatura anual do André - a "Netflix do domador" - onde o assinante tem acesso ao conhecimento completo sobre doma de cavalos: do trabalho de chão à sela e rédea, com conteúdo novo toda semana e contato direto com o André.

### Posicionamento
- **Não é só curso** - é acompanhamento contínuo
- **Não é consultoria individual** - é comunidade com acesso ao expert
- **É o caminho completo** - do iniciante ao domador de sucesso

### Modelo de Negócio

| Item | Detalhe |
|------|---------|
| Ticket | **R$ 597/ano** |
| Parcelamento | 12x de R$ 49,75 |
| Micro-ancoragem | R$ 1,64/dia |
| Meta 1M | ~1.675 assinantes ativos |
| Meta 2M | ~3.350 assinantes ativos |
| Plataforma | Astron Members (app + web) |

### Posição na Esteira
O Doma Pro é a **porta de entrada** do ecossistema. Estrutura futura:

```
[DOMA PRO R$ 597/ano] - Fase inicial
        ↓
   Assinante engajado
        ↓
   Upsells futuros:
   • Cursos avançados (outros temas)
   • Experiências presenciais
   • Mentorias em grupo
   • Certificações
   • Mercado internacional
```

---

## 2. O Que Está Incluído

### Acesso Imediato
**Curso Domador de Sucesso completo** (29 aulas):

| Módulo | Conteúdo |
|--------|----------|
| Mentalidade | Mentalidade de domador, jornada de doma bem feita |
| Confiança e Liderança | Ganhando confiança, entendendo sinais no redondel |
| Dessensibilização | Bandeira, lona, corda, maneia |
| Tempo do Cavalo | Respeitar o tempo, não pular etapas |
| Selaria | Primeira sela, colocação correta |
| Trabalho de Chão | Pressão e alívio, preparação |
| Montaria | Primeira montada, bridão, rédea |
| Controle Corporal | Quebrar nuca, paleta, costela, garupa |
| Verificação | Exercício Xeque Mate |
| Extras | Noções de muares, noções gerais |

### Conteúdo Novo Semanal
**3 vídeos por semana** (12/mês, 144/ano):
- Pílulas práticas de doma - curtas e objetivas
- Respostas a dúvidas dos membros
- Casos reais, correções, técnicas específicas

### Grupo Fechado no WhatsApp
- Comunidade de domadores
- Acesso direto ao André
- Troca entre membros

### Lives Exclusivas
- Plantões de dúvidas sobre doma
- Demonstrações ao vivo
- Convidados especiais

### Certificado de Conclusão
Ao completar todo o curso, o membro recebe o **Certificado de Domador pelo Método André**.

**Requisito:** Completar todas as etapas do curso Domador de Sucesso

**Benefícios do certificado:**
- Prova social ("Sou Domador Certificado pelo Método André")
- Diferencial no mercado de trabalho para domadores profissionais
- Reconhecimento da dedicação e aprendizado

---

## 3. Proposta de Valor

### Para Quem É
- **Domador iniciante** que quer aprender do zero com método
- **Cavaleiro** que precisa domar ou corrigir o próprio cavalo
- **Treinador** que quer padronizar fundamentos na equipe
- **Apaixonado por cavalos** que quer entender a doma mesmo sem ser profissional

### Dor Principal
Doma sem método vira afobação: cavalo tenso, resistência, acidentes e frustração. O cara assiste vídeo solto no YouTube, pula etapa, e o cavalo não responde.

### Promessa (SMART)

**"Domine cada etapa da doma com método, do chão à sela, com acompanhamento contínuo."**

| Critério | Aplicação |
|----------|-----------|
| **Específico** | Doma de cavalos - do trabalho de chão até rédea |
| **Mensurável** | Etapas claras de progresso (abaixo) |
| **Atingível** | Passo a passo sem pular etapa, no ritmo do cavalo e do aluno |
| **Relevante** | Cavalo mais leve, confiante e seguro para qualquer finalidade |
| **Temporal** | Acompanhamento contínuo enquanto for assinante |

### Etapas Mensuráveis de Progresso
- ✓ Cavalo com atenção e foco no treinador
- ✓ Dessensibilizado (aceita bandeira, lona, corda, maneia)
- ✓ Respondendo à pressão e alívio
- ✓ Aceitando sela sem reação
- ✓ Primeira montada segura
- ✓ Respondendo ao bridão e rédea
- ✓ Corpo organizado (nuca, paleta, costela, garupa)

### Por Que Assinar (e não só comprar curso)

| Curso Avulso | Doma Pro |
|--------------|----------|
| Assiste sozinho | Comunidade + André junto |
| Conteúdo parado | 3 vídeos novos por semana |
| Dúvida fica sem resposta | Grupo + lives para tirar dúvida |
| Comprou, esqueceu | Acompanhamento contínuo |

---

## 4. Oferta e Preço

### Ancoragem de Valor

| O que o assinante recebe | Valor se comprasse separado |
|--------------------------|----------------------------|
| Curso Domador de Sucesso completo (29 aulas) | R$ 497 |
| 144 vídeos novos por ano (3/semana) | R$ 997 |
| Acesso ao grupo fechado com o André | R$ 497 |
| Lives exclusivas e plantões de dúvidas | R$ 297 |
| Certificado de Conclusão | R$ 197 |
| **Valor total** | **R$ 2.485** |

### Preço Doma Pro

| | |
|--|--|
| ~~De R$ 2.485~~ | Ancoragem |
| **Por R$ 597/ano** | Preço real |
| **12x de R$ 49,75** | Parcelado |
| **R$ 1,64/dia** | Menos que um café |

### Garantia
**7 dias de garantia incondicional**
- Entrou, não gostou, devolvemos 100%
- Sem perguntas, sem burocracia
- Remove o risco da decisão

### Bônus Sugeridos
- Checklist de etapas da doma (PDF)
- Aula extra: "Os 5 erros que travam a doma"
- Acesso antecipado a novos conteúdos

---

## 5. Objeções e Respostas

### "É caro"
R$ 49,75/mês é menos que uma saca de ração. Você investe milhares no cavalo - investir R$ 1,64/dia no conhecimento pra domar ele direito é o que faz diferença no resultado.

### "Não tenho tempo"
Os vídeos são curtos e práticos. Você assiste no celular enquanto toma café ou antes de ir pro piquete. 15 minutos por dia já muda seu jogo.

### "Meu cavalo é muito difícil/bravo"
O método começa pela base - confiança e dessensibilização. Não importa se é chucro ou problemático. Cavalo difícil é cavalo que não teve base. A gente constrói junto.

### "Já vi muito conteúdo no YouTube"
Conteúdo solto não dá resultado. O Doma Pro é método com sequência, do chão à sela, com acompanhamento. Você não fica sozinho tentando juntar peças.

### "E se não funcionar pra mim?"
Você tem 7 dias de garantia. Entrou, viu que não é pra você, pede reembolso e devolvemos 100%. O risco é zero.

### "Consigo aprender sozinho"
Pode até conseguir, mas em quanto tempo? Com quantos erros? O Doma Pro encurta o caminho - você aprende o que funciona, evita o que não funciona, e ainda tem o André e a comunidade pra tirar dúvida.

---

## 6. Estrutura da Página de Vendas

### 1. Hero (topo)
- **Headline:** "Domine cada etapa da doma com método, do chão à sela"
- **Subheadline:** "Acesso ao curso completo + 3 vídeos novos por semana + grupo fechado com o André"
- **Vídeo:** André falando direto ("Fala primos...")
- **CTA:** "Quero entrar no Doma Pro"

### 2. Seção "Pra quem é"
- Domador iniciante que quer método
- Cavaleiro que precisa domar o próprio cavalo
- Treinador que quer padronizar fundamentos
- Cards visuais com ícones

### 3. Seção "O Problema"
- Doma sem método = afobação, cavalo tenso, acidentes
- "Você assiste vídeo solto, pula etapa, e o cavalo não responde"
- Tom: empatia, sem julgamento

### 4. Seção "O Método"
- Timeline visual das etapas:
  **Sinais → Dessensibilização → Chão → Sela → Rédea → Corpo organizado**
- Mostrar que é progressão, não salto

### 5. Seção "O que você recebe"
- Curso completo (29 aulas)
- 3 vídeos novos por semana
- Grupo fechado no WhatsApp
- Lives exclusivas
- Certificado de conclusão
- Cards com ícones + descrição curta

### 6. Seção "Quem é o André"
- Foto/vídeo
- Credenciais e história
- Tom: próximo, "fala primos"

### 7. Seção "Depoimentos"
- Vídeos ou prints de alunos
- Resultados reais

### 8. Seção "Oferta"
- Ancoragem (~~R$ 2.585~~)
- Preço real: R$ 597/ano ou 12x R$ 49,75
- Garantia 7 dias
- CTA forte

### 9. FAQ
- Objeções transformadas em perguntas
- Respostas curtas e diretas

### 10. CTA Final
- Repetir oferta
- Urgência (se houver): "Vagas limitadas" ou "Preço de lançamento"

---

## 7. Próximos Passos para Lançar

### Fase 1: Preparação

| Tarefa | Descrição |
|--------|-----------|
| Configurar Astron Members | Criar área, subir curso Domador de Sucesso, personalizar visual |
| Definir gateway de pagamento | Integrar com Astron (Hotmart, Kiwify, Guru, etc.) |
| Criar grupo WhatsApp | Grupo fechado para assinantes |
| Gravar vídeo de vendas | André gravando o pitch ("Fala primos...") |

### Fase 2: Página de Vendas

| Tarefa | Descrição |
|--------|-----------|
| Construir landing page | Seguir estrutura da seção 6 |
| Escrever copy | Headlines, bullets, objeções |
| Coletar depoimentos | Alunos atuais dos cursos |
| Configurar checkout | 12x de R$ 49,75, garantia 7 dias |

### Fase 3: Conteúdo Recorrente

| Tarefa | Descrição |
|--------|-----------|
| Criar calendário de conteúdo | 3 vídeos/semana - definir temas do primeiro mês |
| Gravar estoque inicial | Pelo menos 2-4 semanas adiantadas |
| Definir dia das lives | Frequência e horário fixo |

### Fase 4: Lançamento

| Tarefa | Descrição |
|--------|-----------|
| Definir estratégia | Lançamento aberto, perpétuo ou com janelas? |
| Aquecer audiência | Stories, posts, lives de pré-lançamento |
| Abrir carrinho | CTA em todos os canais |
| Onboarding | Sequência de boas-vindas para novos assinantes |

---

## Resumo Executivo

| Item | Detalhe |
|------|---------|
| **Produto** | Doma Pro - assinatura anual de doma de cavalos |
| **Preço** | R$ 597/ano (12x R$ 49,75) |
| **Plataforma** | Astron Members |
| **Conteúdo inicial** | Curso Domador de Sucesso (29 aulas) |
| **Conteúdo recorrente** | 3 vídeos/semana + lives + grupo WhatsApp + certificado |
| **Garantia** | 7 dias incondicional |
| **Meta inicial** | ~1.675 assinantes = R$ 1M/ano |
| **Posição na esteira** | Produto de entrada - porta para upsells futuros |

---

*Documento gerado em 25/01/2026 | Atualizado em 25/01/2026*
